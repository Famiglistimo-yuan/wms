...
- **客户端禁止任何 JDBC/数据库访问**：数据一律走 HTTP 调 server
...
- **git 写操作必须先经人工确认**：commit、push、merge、rebase、分支创建/删除、`git reset` 等一律先向人说明将要执行的命令并获得同意；只读命令（status/log/diff/branch）不受限。同意必须针对该写操作本身、逐项给出：对方案中某个子问题（如分支基点、文件位置）的回答不构成对其他写操作（尤其 commit）的授权；commit 须在展示最终 diff 后获得单独的明确同意
- **创建或修改项目文件前必须先展示内容并获得同意**：新文件（含配置、工作流、脚本）先给出完整内容与放置位置，修改现有文件先给出 diff 预览，经确认后才写入磁盘；纯咨询、讲解类回答除外
- **提交/推送前必须自查敏感信息**：暂存区不得含密码、令牌、个人日志或本机路径；本仓库为 **public**，内容一旦推入即永久公开（git 历史不可撤回）。凡新增文件先 `git diff --staged` 过目再提交
